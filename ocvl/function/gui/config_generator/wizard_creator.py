...
# Create bold text wrapper
bold = QtGui.QFont()
bold.setBold(True)

# Boolean helper function that checks for "{Modality}} in a string
...
class MainWizard(QWizard):
    def __init__(self, parent=None):
        ...
        # Add page classes to wizard and set ID numbers
    ...
    # If import radio button is selected, update next to "Import >"
...
# Intro page class, where you select to either create new JSON or import an existing
class IntroPage(QWizardPage):
    ...
    def nextId(self):
        # If create button is selected, skip import and move onto next page
        if self.create_button.isChecked():
            return 1
        # Else, import JSON and create imported_config variable, move to import generation
...
# Selection page class, where you select the kind of generation you want
class SelectionPage(QWizardPage):
    ...
    def nextId(self):
        # Proceed to page 2 if simple generation is selected
        if self.simple_button.isChecked():
            return 2
        # Else proceed to advanced setup page
        elif self.adv_button.isChecked():
            return 5
        return 2

# Version Description page class, where the user chooses version and description of configuration file
class VerDescriptionPage(QWizardPage):
    def __init__(self, parent=None):
        ...
        # Create container widget for scrollable area
        container = QWidget()
        scroll.setWidget(container)
...
# Preanalysis class page, where the user sets up pre-analysis pipeline configuration
class PreanalysisPage(QWizardPage):
    ...
    # Changes availibility of align ref modality widget
    ...
    # Updates list of available elements available in group by editor
    ...
    # Changes availibility of modality widget
